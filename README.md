...
└── web-rpa/             # Firebase project (Hosting + Functions + Rules)
    ...
    ├── public/servers.html  # Live Server Map — ผังว่าบอทตัวไหนรันบน server ไหน
    ├── public/rpa-common.js # ตรรกะที่หน้าต่างๆ ใช้ร่วมกัน (normalize / stale / เรียง / escape)
...
- **History Logs** — log viewer แบบ terminal: เลือกช่วงเวลา (วันนี้ / 24 ชม. / 7 / 30 วัน / กำหนดเอง) แล้ว query ตรงจาก Firebase,
  เลื่อนดูต่อเนื่องได้ถึง **50,000 รายการ** ด้วย virtual scroll (วาง DOM แค่ ~40 แถว), แถบกิจกรรมบอกช่วงที่รันเยอะ/พังเยอะ,
  live tail แถวใหม่ไหลเข้าด้านบนพร้อมกะพริบ, ฟิลเตอร์ Status / ชื่อ Bot / ค้นใน stage+error, Export CSV ตามที่กรองไว้
...
- **🗺️ Live Server Map (`servers.html`)** — ผังว่าบอทตัวไหนรันบน server เครื่องไหน (ดูหัวข้อด้านล่าง)
- **ธีม Dark / Light** — ดีไซน์ professional, จำธีมที่เลือกไว้, ตัวบอกสถานะการเชื่อมต่อ (LIVE/offline) บนแถบบน

> **หมายเหตุเรื่อง KPI "Success Rate วันนี้"** — มี listener ของตัวเองที่ query log **ตามวันที่** (`startAt(วันนี้)`) แยกจาก log ของแท็บ History
...
> แยกกันไว้แบบนี้ทำให้ KPI ไม่ขึ้นกับช่วงเวลาที่เลือกในแท็บ History ด้วย
...
ประวัติมี 150,000+ record แล้วและโตทุกวัน การแบ่งหน้าละ 25 แถวใช้ไล่หาอะไรไม่ไหว
แท็บ History จึงเป็น log viewer แบบเดียวกับที่ใช้ดู log ระบบ

| | |
|---|---|
| **ช่วงเวลา** | ปุ่มเลือกช่วง = query จริงที่ยิงไป Firebase (`startAt`/`endAt`) ไม่ใช่กรองทีหลัง เพดาน 50,000 รายการต่อ query (~9 MB) ถ้าชนเพดานจะมีป้ายบอกว่าเห็นไม่ครบช่วง |
| **Virtual scroll** | วาง DOM เฉพาะแถวที่มองเห็น (~40 แถว) ไม่ว่าจะโหลดมากี่หมื่นแถว |
| **แถบกิจกรรม** | ปริมาณ log ต่อช่วงเวลา แท่งแดงคือ Failed — เห็นได้ทันทีว่าปัญหากระจุกตัวช่วงไหน |
| **Live tail** | log ใหม่ไหลเข้าด้านบนแล้วกะพริบ · เลื่อนลงไปอ่านอยู่จะหยุดเลื่อนอัตโนมัติ แล้วขึ้นปุ่ม "N รายการใหม่" ให้กดกลับขึ้นบน |
| **ฟิลเตอร์** | Status / ชื่อ Bot / ค้นข้อความใน stage + error — ทำในเครื่องบนชุดที่โหลดมาแล้ว จึงตอบสนองทันที |

**log โหลดตอนเปิดแท็บ History หรือ Analytics เท่านั้น** — เดิมหน้า Dashboard ดึง log หลายเมกะไบต์ตั้งแต่เปิดหน้า
ทั้งที่คนส่วนใหญ่เข้ามาดูแค่หน้า Live

---

## 🗺️ Live Server Map (`servers.html`)

ผังว่า RPA แต่ละตัวรันอยู่บน server เครื่องไหน — ย้ายมาจากไฟล์ draw.io ที่ทีมเคยต้องส่งไฟล์กันเอง
ตอนนี้เก็บบน Firebase แล้ว ทุกคนเปิดดูเห็นตรงกัน และแก้พร้อมกันได้แบบ real-time

| ทำอะไรได้ | |
|---|---|
| **ดูผัง** | แถวละ 1 server → เส้นที่มี packet วิ่งรับ-ส่ง → กล่องรายชื่อบอทของเครื่องนั้น |
| **ย้ายบอทข้ามเครื่อง** | ลากการ์ดบอทไปวางในกล่องของ server อื่น บันทึกทันที |
| **เพิ่ม/แก้/ลบ** | ทั้ง RPA Bot และ Server — กัน server ที่ยังมีบอทอยู่ไม่ให้ถูกลบ |
| **ค้นหา** | กรองตามชื่อบอท ชื่อ server หรือหมายเหตุ |
| **สถานะ** | `ใช้งานจริงแล้ว` / `ยังไม่ขึ้น` (ตรงกับตัวอักษรสีน้ำเงินในไฟล์ draw.io เดิม) |
| **กล่อง Archived** | บอทที่เลิกรันแล้ว ลากมาเก็บที่กล่องล่างสุด — จดให้ด้วยว่า**เคยรันบนเครื่องไหน**และ**เก็บเมื่อไหร่** ลากกลับไปเครื่องจริงได้ถ้ากลับมาใช้อีก |

**ครั้งแรกที่เปิด** ผังจะว่าง มีปุ่มนำเข้ารายการตั้งต้น 6 servers / 25 bots จาก draw.io ให้กดครั้งเดียว
(ต้อง login ก่อน เพราะเป็นการเขียนข้อมูล)

**หมายเหตุ**

- ยัง**ไม่ผูกกับ log ที่บอทยิงเข้ามา** — เป็นผังที่คนกรอกเอง ตั้งใจให้เป็นแบบนี้ในเฟสแรก
  โครงสร้างข้อมูลเผื่อฟิลด์ `log_key` ไว้แล้ว ถ้าอนาคตอยากโชว์สถานะจริงบนผังก็ต่อได้เลย
- animation ปิดได้จากปุ่มบนแถบบน และหยุดเองเมื่อสลับไปแท็บอื่น หรือเมื่อเครื่องตั้งค่า reduced-motion
- **drag & drop ใช้ HTML5 DnD จึงไม่ทำงานบนมือถือ/แท็บเล็ต** — บนจอสัมผัสให้ย้ายผ่านปุ่มแก้ไข
  แล้วเปลี่ยน dropdown "รันบน Server" แทน (ได้ผลเหมือนกัน)
- ข้อมูลอยู่ที่ node `rpa_server_map` — **ต้อง deploy database rules ก่อนใช้งานครั้งแรก**
  ไม่งั้นหน้าจะขึ้นว่าอ่านไม่ได้:

```bash
cd web-rpa; firebase deploy --only database
```
